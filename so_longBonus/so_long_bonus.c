...
/*   Updated: 2024/03/09 17:12:18 by aennaqad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_its_pos_win(t_tool *var)
{
	...
	flood_fill_bns(cpy, var->x, var->y);
	check_only_v_0_1_(cpy);
	clear_2d(cpy);
	get_exit_pos(var, &x, &y);
	cpy = copy_original(var->map, var->line_number, var->size_line);
	flood_fill_bns_e(cpy, x, y);
	check_only_v_0_1_(cpy);
	clear_2d(cpy);
}
...
int	main(int ac, char **av)
{
	...
	check_stranger(var.map);
	check_p_c_e(var, var.map);
